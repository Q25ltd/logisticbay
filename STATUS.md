...
- ⏳ Follow-ups: set `planned` when all stops assigned (D3.2); per-vehicle payload field + double-booking guard (D5.4/D5.5); mobile driver UI for `trailer_swap` (swap button + new-trailer reg prompt — API live since Step 7), for `handover_offered`/`handover_accepted` (offer/accept buttons — API live since Step 8), and for the Step 11 exception events (delay/breakdown/refused/damage buttons) — mobile session. Web cancel disposition prompt ✅ (2026-07-18): the board's ✕ on a loaded run opens a choice dialog (return to collection point / leave at a yard + name) and retries the cancel with it.
