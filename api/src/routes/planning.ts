 ...
 *
 * Planning-board READS and board-specific tools only. All run/assignment WRITES
 * (create, patch, publish, add/remove/resequence stops, cancel) live in
 * routes/runs.ts — the single run system (LOAD_MOVEMENT_PLAN Step 16).
 *
 * GET  /planning/unplanned?date=YYYY-MM-DD   — stops ready to plan, clustered by location
 * GET  /planning/propose-runs?date=          — run proposals (advisory)
 * GET  /planning/runs?date=YYYY-MM-DD        — runs for that date (board-shaped read)
 * POST/DELETE /planning/runs/:id/waypoints   — non-job waypoints (depot/yard/overnight)
 * POST /planning/runs/:id/overnight-rest     — composite: create the follow-on relay run
 * GET  /planning/fleet?date=YYYY-MM-DD       — available trailers + trucks for that date
 * GET  /planning/work-items?dateFrom&dateTo  — planner work items
...
import { generateRunReference, recalculateDerivedRequirements } from "../services/runService.js";
