      ...
      await tx.jobTemplate.create({
        data: {
          ...
          defaultJobData:      JSON.parse(JSON.stringify({
            ...
            serviceType: body.serviceType ?? "", priority: body.priority ?? "normal",
            billingNotes: body.billingNotes ?? "", custRefRequired: body.custRefRequired ?? false,
            poRequired: body.poRequired ?? false, goodsDescription: body.goodsDescription ?? "",
            quantity: body.quantity != null ? String(body.quantity) : "", quantityUnit: body.quantityUnit ?? "",
            hazardClass: body.hazardClass ?? "", podRequired: body.requirePOD ?? true,
            vehicleCategory, minGvwClass, bodyTypes, equipment, trailersAllowed,
            vehicleAccessNotes: body.vehicleAccessNotes ?? "",
            failureAction: body.failureAction ?? "call_assistance",
            assistancePhone: body.assistancePhone ?? "", assistanceNote: body.assistanceNote ?? "",
          })),
          qualityScore: quality.score,
          status:       "active",
        },
...
type ExistingJobStop = {
  ...
  town: string | null; postcode: string | null; locationTextSnapshot: string;
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    return tx.job.update({
      ...
      data: {
        ...
        customerName:        patchCustomerName,
        plannerNotes:        p(body.plannerNotes, job.plannerNotes),
        vehicleCategory,
        minGvwClass,
        bodyTypes:           jsonOrNull(bodyTypes),
        equipment,
        ...
        serviceType:         p(body.serviceType, job.serviceType),
        customerRef:         p(body.customerRef, job.customerRef),
        purchaseOrderNumber: p(body.purchaseOrderNumber, job.purchaseOrderNumber),
        billingNotes:        p(body.billingNotes, job.billingNotes),
        bookingContactName:  p(body.bookingContactName,  job.bookingContactName),
        bookingContactPhone: p(body.bookingContactPhone, job.bookingContactPhone),
        bookingContactEmail: p(body.bookingContactEmail, job.bookingContactEmail),
        custRefRequired:     p(body.custRefRequired, job.custRefRequired),
        poRequired:          p(body.poRequired, job.poRequired),
        vehicleAccessNotes:  p(body.vehicleAccessNotes, job.vehicleAccessNotes),
        failureAction:       p(body.failureAction, job.failureAction),
        assistancePhone:     p(body.assistancePhone, job.assistancePhone),
        assistanceNote:      p(body.assistanceNote, job.assistanceNote),
        approvalContactName:           p(body.approvalContactName,           job.approvalContactName),
        approvalContactPhone:          p(body.approvalContactPhone,          job.approvalContactPhone),
        alternativeReturnAddress:      p(body.alternativeReturnAddress,      job.alternativeReturnAddress),
        alternativeReturnPostcode:     p(body.alternativeReturnPostcode,     job.alternativeReturnPostcode),
        ...
        goodsDescription:    p(body.goodsDescription, job.goodsDescription ?? ""),
        goodsType:           p(body.goodsType, job.goodsType ?? ""),
        quantity:            body.quantity !== undefined ? toNullableNumber(body.quantity) : job.quantity,
        quantityUnit:        p(body.quantityUnit, job.quantityUnit ?? ""),
        ...
        dimensions:          p(body.dimensions, job.dimensions ?? ""),
        ...
        tempRange:           p(body.tempRange, job.tempRange ?? ""),
        hazardClass:         p(body.hazardClass, job.hazardClass ?? ""),
