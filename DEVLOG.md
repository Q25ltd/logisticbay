...
## The driver's half of the loop: hazards, navigation, and a way to report trouble 2026-07-26 (mobile driver session, tasks #22–25)

With the execution engine fixed earlier the same day, the planner→driver→planner circuit carried *progress* but nothing else: the driver couldn't see what they were carrying, couldn't get directions to the gate, and — despite S7/S8/S11 being live and tested server-side since mid-July — had no way at all to tell the office something had gone wrong. The Live screen's needs-review queue was built precisely for those signals and was receiving none.

**Nothing here needed an API change.** `GET /jobs/:id` and `GET /jobs/my` `include` the whole Job model and the full stops array, so hazard class, tunnel code, temperature range, special requirements, equipment, driver-visible notes, entrance pins, site contacts and navigation instructions were all already on the wire. Every one of them was simply never rendered. Worth remembering as a pattern: the earlier phantom-field bugs were the app reading fields that didn't exist; this was the mirror image — real fields nobody displayed.

**Safety (#22).** `SafetyCard` sits directly under the progress bar, because hazard information a driver has to scroll for is information they don't have. ADR shows the class with UN number, proper shipping name, packing group and tunnel category pulled from the `loadData` blob. Temperature, fragile, high-value, oversized, escort-required, equipment and weight follow, ordered danger→warn→info. `plannerNotes`/`internalNotes` are deliberately excluded (ARCHITECTURE: office-only); `driverVisibleNotes` and `vehicleAccessNotes` are shown in full. Every stored token is mapped to plain language — no `dangerous_goods` reaches the screen, per the standing no-dev-language rule. The card renders nothing at all when a job carries nothing special, so it never becomes furniture drivers scroll past.

**Navigation (#23).** The old Route card showed one site name at each end. `StopsCard` replaces it with every stop in sequence: address, booked time or window, tap-to-call site contact, the reference the site will ask for, the planner's own "getting in" instructions, and a Navigate button. It prefers the form's `lat`/`lng` over the address and labels the button "Navigate to entrance" when it has the pin — PRODUCT decision 5 exists because sending an artic to a postcode centroid puts it on a residential street. Android falls back to a Google Maps URL when nothing handles `geo:`.

**Reporting (#24).** `ReportSheet` offers running late, breakdown, damage, refused delivery, swap trailer, drop at yard, pick up from yard, offer handover and take over handover. Each appears **only** from the execution states the server accepts it from, mirrored from `EVENT_DEFINITIONS.allowedFromStates`, so a button the driver can see never fails. Everything routes through the offline queue regardless of connectivity — not as a fallback but because it's the only path: `EVENT_TYPE_MAP` on the online status route carries just the five happy-path statuses, and none of these events can travel it. It also happens to be correct on its own terms, since the driver reporting a breakdown is the least likely to have signal.

*Judgement call worth recording:* `damage_writeoff` is **not** offered to drivers. The API accepts it from them, but it is terminal and financial — the load moves to `written_off` and the quantity leaves the ledger — and B13 frames that as the planner's decision. A driver reports damage; the office decides whether it's a write-off. Revisit only with a deliberate product decision.

**Push (#25).** S14 has been dispatching `run_published` and `run_recalled` into a void since 2026-07-17 — no device had ever registered a token. `lib/push.ts` now registers with `POST /devices` at sign-in *and* at app start (tokens rotate; a driver signed in for weeks would otherwise go quiet). Added `expo-notifications` + `expo-device`, Expo-pinned for SDK 54, plus the plugin in app.json. **It is dormant**: there is no `eas.json`, and push needs an Apple APNs key and Google FCM credentials against the EAS project before anything arrives (the `projectId` is already there). Registration fails quietly by design — nobody is blocked from working because notifications aren't set up. User was asked before the dependency went in (SAFETY stop-before-risk covers package manager + mobile config) and chose "write the code now, dormant".

**A pre-existing dependency conflict surfaced during install**, left alone deliberately: `react-dom@19.2.7` is in the tree demanding `react ^19.2.7` while the project correctly pins `react@19.1.0` for Expo SDK 54 — and `react-dom` isn't even a declared dependency. Installed with `legacy-peer-deps` so no existing version moved. `expo install --check` also lists expo-location, expo-router, expo-updates, react-native-get-random-values and both react-navigation packages as off their SDK-54 versions. All pre-existing; worth a dedicated dependency-hygiene pass.

**New API test** `driverReportedEvents.test.ts` (7 subtests) pins the client contract rather than re-testing behaviours that already have scenario coverage: each action is accepted from the state the app offers it from, the driver's own words reach the event note, state-preserving events genuinely leave the driver's step alone, GPS pins a breakdown, `newTrailerReg`+`yardRef` are honoured, an unknown trailer reg is flagged rather than refused, and a yard drop reconciles to `collected` not `completed`.

Gates: api+web typecheck 0 ✅ · mobile `tsc --noEmit` 0 ✅ · `expo config` valid ✅ · api tests **324/324** ✅ · check:vocab ✅ · check:docs ✅ · web build ✅ · knip baseline ✅ (two of my own new exports were flagged and removed rather than left). **Not device-verified** — the UI needs a simulator or a real handset, which this session had no way to drive.

Still open on the driver app: POD is a typed reference number only — `DeliveryForm` has no signature or photo capture, while `JobPart.proofRequirements` can already demand both, so a job can be marked delivered without the evidence it asked for. STATUS.md had claimed signature/photo were done; corrected in `65e88c3`.
...
User directive codified in ARCHITECTURE.md § "Four intake gates": all data is born at PRF / CJP / driver registration / fleet registration; algorithms may only consume form-born data. Root cause of the "flatbed allocated for a liquid load" class of bug confirmed in the dev DB: trailer "Trans 28" has `bodyType: ""` + free-typed `trailerType: "Flatbed"`, and units KT17HGF/FX17DVR have completely blank identity (no category/GVW/body) — rows that predate the routes' taxonomy checks. **These rows must be re-registered via the fleet form** (the system cannot invent their truth).

**Fleet gate hardened** (`api/src/schemas/fleet.ts` new, wired into all four create/patch routes — the routes already had taxonomy checks via `isBodyCategory`/`isGvwClass`/`isBodyType`; correction of the 2026-07-13 claim that fleet had "no validation"): Zod shape + `.max()` caps everywhere, **status enums** from new registry `api/src/constants/fleetVocab.ts` (client can no longer write arbitrary status — including the previously-possible `"deleted"`), **dimension sanity bounds** (heightM ≤ 8 etc. — catches cm-typed-as-m), and **`trailerType` is now always persisted as the canonical bodyType** (it's a legacy display alias that token-matching algorithms read — never stores free text again; PATCHing bodyType self-heals old rows).

**Shift trailer ownership** (the "driver hooks up a trailer the system doesn't know" side door): `ShiftSegment.trailerOwnership` added (migration `20260714000000_shift_segment_trailer_ownership`, hand-written SQL + local `db push` — `migrate dev` wanted a destructive reset, refused). Segment create matches `trailerReg` against the company fleet: ours → `company` (server-set); unknown + driver claim → `contractor`/`third_party` (Zod enum allows only these two from the client); unknown + no claim → `unregistered` + human warning in the response — **never blocks** (segments are posted at end-of-shift review; blocking would risk losing a day's data). New driver-accessible `GET /fleet/trailers/lookup?reg=` so the app can ask "contractor or third party?" at vehicle-setup time (mobile UI prompt = separate mobile session, tracked in QUESTIONS.md).

Also deduped: `types/requests.ts` shift interfaces (CreateShiftBody/CreateSegmentBody/CheckItem/CreateDeliveryBody/SubmitShiftBody) deleted — the `z.infer` exports in `schemas/shifts.ts` are the single source (the route was importing the stale hand-written twins).

Gates: typecheck 0/0 · vocab ✅ · api tests **235/235** (new `fleetIntake.test.ts`: 15 subtests — garbage bodyType/status/deleted-status/cm-dims rejected, canonical + legacy-mapped creates, PATCH self-heal, driver lookup + tenant scoping, ownership company/unregistered+warning/contractor, driver-cannot-claim-company) · build ✅ · knip exports 119 = baseline, types 77 (−5 vs baseline after dedup).

## Repo-wide audit + cleanup: flaky test gate fixed, 17 dead files deleted, zero `any` in web 2026-07-13

Full-system verification pass requested by the user ("run check to all system, find mistakes, fix, clean"). Three real problems found and fixed:

**(1) The api test gate was silently dropping suites — the most serious find.** `npm test --prefix api` used `--test-force-exit`, which nondeterministically discarded 6–24 tests per run while still reporting green (observed 196/200/203/212/214 across runs; two consecutive runs proven to execute *different, overlapping* suite sets — e.g. one run dropped `checkLoadMixing` + `getPlannerWorkItems`, the next dropped drivers'-hours + vehicle-suitability + GPS suites; all "pass 0 fail 0 skipped"). A failing test in a dropped suite would have been invisible, locally AND in CI (which runs the same script). Removed `--test-force-exit`: the suite completes cleanly in ~4.5s with **220/220 deterministic** (verified three consecutive runs). Every historical DEVLOG test count (196–203) was an undercount of a then-smaller-but-still-truncated suite.

**(2) 17 dead files deleted** (per CLAUDE.md "never imported → delete on sight"): the legacy planner-dashboard cluster left behind when the "remove legacy job-level driver assignment" refactor removed its last imports — `AssignDrawer.tsx`, `DriverSnapshot.tsx`, `FleetSnapshot.tsx`, `JobCard.tsx`, `JobDetailDrawer.tsx`, `QuickFixDrawer.tsx`, `dashboardConstants.ts`, `dashboardTypes.ts`, `dashboardUtils.tsx` — plus `StopCard.tsx`, `components/Card.tsx`, `api/dashboard.ts`, `api/locations.ts`, `api/templates.ts`, `lib/textCase.test.ts` (a test no runner ever executed; web has no test script), and api `schemas/fleet.ts` + `schemas/locations.ts` (Zod schemas written but never wired into their routes). Also removed `jobsApi.allocate` — it called `PATCH /jobs/:id/allocate`, an endpoint that does not exist in the API (only caller was the dead AssignDrawer). This closes the B6 "third assignment system" follow-up: Run-based allocation is now the only path. knip unused files 25 → 8 (all 8 remaining are deliberate vocab/taxonomy mirrors + mobile-session files).

**(3) Zero `any` left in web/src** (CLAUDE.md rule; was 39 `as any` + 25 `: any`). Almost all `as any` casts were stale — the fields (quantityUnit, hazardClass, tempControlled, loadData, bodyTypes, siteName/town/postcode on stops) already exist on the `Job`/`JobPart` types. Genuinely missing fields added to web `Job` from schema.prisma: `tunnelCode`, `alternativeReturnSiteName/AddressLine2/Town/County/Country/Lat/Lng/NavigationInstructions`, `rejectionSignatureRequired`, `rejectionNotes`. API client methods typed to their real response shapes (`driversApi.create`/`resetPin`, `jobsApi.updateStatus`). The 25 `catch (err: any)` blocks converted to `catch (err)` + new `web/src/lib/errorMessage.ts` helper (`err instanceof Error` narrow, matches the existing LoginPage/DashboardPage idiom); PRF's catch narrows `Error & { errors?: string[] }` (the shape `publicPost` actually throws). Type-only — zero runtime behaviour change except empty error messages now fall back to a readable default.

**Discovered, NOT fixed (needs a decision — added to QUESTIONS.md):** `routes/fleet.ts` and `routes/locations.ts` have no Zod validation at all (the dead schemas were apparently written for them and never wired). Wiring them in would change API behaviour (stricter rejection) — deferred rather than silently changed. Remaining known debt: `email.ts`/`pdf.ts` use `: any` for shift shapes (api-side, untyped includes); Sentry TODOs are deliberate P0.2 markers.

Gates: typecheck 0/0 · check:vocab ✅ · api tests **220/220 ×3 runs** · web production build ✅ · prisma validate ✅ · knip: unused files 25→8, exports 119→118 baseline, nothing new. Browser: login + public pages console-clean, no `/auth/*` calls before submit, `/app/dashboard` unauthenticated → redirects to login. Authenticated-page click-through not performed (agent cannot enter credentials); those changes are type-only and covered by build+typecheck.

**Follow-up (same day): `api/scripts/` was invisible to every gate and rotten.** User's editor showed `prisma.plannedJob` errors in the scripts — root cause: `api/tsconfig.json` included only `src`, so the CLI typecheck (and CI) never saw `scripts/`; VS Code checks any open file regardless. 14 errors across 7 of 9 scripts, dating to the `PlannedJob → Job` schema refactor (`84118c2`). Resolution: **deleted the 5 one-time migration scripts** (`backfill_stops_v2` — targets the *deleted* JobRequest model, can never run again; `backfill_vocab_v1`, `backfill_phase1_runs`, `migrate_template_job_data`, `backfill_job_stop_type` — all single-use, already executed, recoverable from git) + their `backfill:*` package.json entries; **fixed the 4 live dev utilities** (`check_db`, `check_users`, `patch_seed_refs`, `seed_sample_jobs`: `plannedJob→job`, `materialType→goodsType`, `quantityExpected→quantity` (string→Number), dropped `plannedDate` (removed from schema — date derives from stop `timeWindowStart`, which the seed already sets), removed an `as any`); **added `scripts` to `api/tsconfig.json` include** so this can't silently rot again. Known knip delta vs main: `licencesThatCanDrive` in `api/src/constants/vehicleTaxonomy.ts` shows unused (only importer was the deleted `backfill_vocab_v1`) — cannot be un-exported without desyncing the byte-identical vocab mirror, so it joins the accepted mirror-noise baseline. Gates re-run: typecheck 0/0 (scripts now included) · vocab ✅ · tests 220/220 · build ✅.

## Runs — conflict awareness: double-booking, driver↔vehicle attachment, full trailers 2026-07-12 (same day)

User's four ops questions answered in code. **(1) Double-booking**: `PATCH /runs/:id` now warns on same-day truck/trailer reuse ("Truck is already on run RUN-x that day") alongside the existing driver warning — warn not block, because sequenced same-day reuse and relay runs are legitimate; the picker already disables busy assets. The web page previously **discarded** the PATCH `warning` field — inline assignment now surfaces it in a dismissible amber banner. **(2) Driver↔vehicle attachment**: truck candidates now know `DriverProfile.defaultTruckReg` — the assigned driver's usual unit is **recommended first** with note "assigned driver's usual unit", and any other driver's usual unit is noted "usually Bob's unit" (selectable — a visible choice, not a block). New informational `note` field on Candidate (kept out of `reasons`, which is failure-only and drives the fit score). **(3+4) Full trailers**: candidates + readiness are now load-state aware via `FleetTrailer.status="loaded"` + `linkedJobId` — loaded with **another** job → unavailable in the picker ("loaded with another job — full"), warned at assign time, and **hard-blocks publish** (`trailer_load_state` fail); loaded with **this run's job** → the RIGHT trailer: available, labelled "loaded with this job", and recommended over empty ones (previously wrongly excluded!). Pure tests +5 (20 total across the two services). Gates: typecheck 0/0 · vocab ✅ · api tests 202/202.

## Runs — yard-grab trailer semantics in the B5 gate 2026-07-12 (same day)

User correction from ops reality: drivers often just take a suitable trailer from the yard — pinning one at planning time is optional. Changed in `runReadinessService`: **missing trailer = soft warn, never a blocker**; the warning text carries the required trailer TYPE in human words ("Needs: temperature-controlled (fridge)" / the jobs' `trailersAllowed` as labels via `bodyTypeLabel`) so the planner sees it on the Runs card and the driver knows what to collect. The registration mechanism already exists — `POST /shifts` accepts `trailerReg` + trailer walkaround checks, and mobile has `ChangeVehicleScreen`; `GET /jobs/my` already returns `trailersAllowed`/`tempControlled`/`hazardClass` for the driver app to display (mobile UI rendering = a mobile session). Tightened the other side: **driver-not-trailer-rated is now HARD whenever the load needs a trailer**, even with none pinned (he'll be pulling one either way) — previously only checked once a trailer was assigned. A pinned-but-incompatible trailer remains a hard fail. Pure tests updated (blocked→warn case replaced + not-rated-no-trailer case added); integration: artic run with no trailer blocked while driver unrated → publishes once rated, readiness warn carries "fridge". Gates: typecheck 0/0, api tests 196/196.
