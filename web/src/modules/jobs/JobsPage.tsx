...
function JobRow({ job, onStatusChange, onNote }: {
  ...
  return (
    <tr className={"hover:bg-gray-50 transition-colors " + (hasNote ? "bg-yellow-50" : "")}>
      <td className="px-4 py-3 text-sm text-primary">{fmt(job.plannedDate)}</td>
      <td className="px-4 py-3">
        <div className="text-sm font-medium text-primary truncate max-w-48">{job.pickupTextSnapshot || "—"}</div>
...
function CreatePanel({ drivers, templates, date, onClose, onCreated }: {
  ...
  function applyTpl(id: string) {
    ...
    if (!t) return;
    setF(p => ({ ...p, templateId:id, pickupTextSnapshot:t.pickupTextSnapshot,
      dropoffTextSnapshot:t.dropoffTextSnapshot, referenceNumber:t.defaultReference||p.referenceNumber,
      materialType:t.defaultMaterialType||p.materialType, plannerNotes:t.defaultNotes||p.plannerNotes }));
  }
      ...
      <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl">
        ...
        <form onSubmit={submit} className="p-6 space-y-3">
          ...
          <div><label className="label">Template</label>
            <select className="input" value={f.templateId} onChange={e=>{setF(p=>({...p,templateId:e.target.value}));applyTpl(e.target.value);}}>
          ...
          <div><label className="label">Driver *</label>
            <select className="input" value={f.assignedDriverId} onChange={e=>setF(p=>({...p,assignedDriverId:e.target.value}))} required>
              <option value="">Select...</option>{drivers.map(d=><option key={d.id} value={d.id}>{d.displayName}</option>)}
            </select></div>
          <div><label className="label">Date *</label>
            <input className="input" type="date" value={f.plannedDate} onChange={e=>setF(p=>({...p,plannedDate:e.target.value}))} required/></div>
          <div><label className="label">Pickup *</label>
            <input className="input" value={f.pickupTextSnapshot} onChange={e=>setF(p=>({...p,pickupTextSnapshot:e.target.value}))} placeholder="15 Arden Place, LU2 7YE" required/></div>
          <div><label className="label">Dropoff *</label>
            <input className="input" value={f.dropoffTextSnapshot} onChange={e=>setF(p=>({...p,dropoffTextSnapshot:e.target.value}))} placeholder="34 Dunelm Road, TS29 6PX" required/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Reference</label>
              <input className="input" value={f.referenceNumber} onChange={e=>setF(p=>({...p,referenceNumber:e.target.value}))} placeholder="REF001"/></div>
            <div><label className="label">Material</label>
              <input className="input" value={f.materialType} onChange={e=>setF(p=>({...p,materialType:e.target.value}))} placeholder="Type 1..."/></div>
          </div>
          <div><label className="label">Planner Notes</label>
            <textarea className="input min-h-16" value={f.plannerNotes} onChange={e=>setF(p=>({...p,plannerNotes:e.target.value}))} placeholder="Call site before arrival..."/></div>
...
export default function JobsPage() {
  ...
  return (
    <div className="p-6">
      ...
      {showCreate && (
        <CreatePanel drivers={drivers} templates={templates} date={date}
