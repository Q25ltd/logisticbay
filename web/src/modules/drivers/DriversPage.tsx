...
function DriverForm({
  ...
  if (loginDetails) return (
    <div>
      <div className="bg-green-50 border-2 border-green-500 rounded-xl p-5 mb-4">
        <div className="font-bold text-green-800 mb-3 text-base">
          📱 Driver Login Details — share with driver
        </div>
        <div className="bg-white rounded-lg p-4 font-mono text-sm space-y-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-2xl font-black tracking-widest bg-yellow-100 px-3 py-1 rounded">
              {loginDetails.pin}
            </span>
          </div>
        </div>
        <p className="text-xs text-green-700 mt-3">
          ⚠ Driver must change this PIN on first login. Save these details now.
        </p>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {!initial && (
        <Input
          label="Email (creates login)"
          type="email"
          value={form.email}
          onChange={set("email")}
          placeholder="driver@company.com"
          hint="If provided, driver can log into the mobile app with a PIN"
        />
      )}
...
// ─── Driver card ──────────────────────────────────────────────────────────────
function DriverCard({
  driver, onEdit, onToggleStatus, onResetPin,
}: {
  driver: Driver;
  onEdit: () => void;
  onToggleStatus: () => void;
  ...
  return (
    <div className={`card p-4 ${driver.status === "inactive" ? "opacity-60" : ""}`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="font-bold text-primary text-base">{driver.displayName}</span>
            <Badge status={driver.status} />
          </div>
          <div className="text-sm text-muted space-y-0.5">
            {driver.employeeNumber && <div>🪪 {driver.employeeNumber}</div>}
            {driver.phoneNumber    && (
              <a href={`tel:${driver.phoneNumber}`} className="block hover:text-accent transition-colors">
                📞 {driver.phoneNumber}
              </a>
            )}
        ...
        <div className="flex flex-col gap-2 ml-4">
          ...
          <button
            ...
            className={`text-xs font-semibold hover:underline ${
              driver.status === "active" ? "text-red-500" : "text-green-600"
            }`}
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div className="bg-green-50 border-2 border-green-500 rounded-xl p-4 mb-4">
          ...
          <p className="text-xs text-green-700 mt-3">
            Driver must change PIN on next login.
          </p>
        </div>
...
export default function DriversPage() {
  ...
  async function handleToggleStatus(driver: Driver) {
    ...
    if (next === "inactive" && !confirm(`Deactivate ${driver.displayName}? They won't receive new jobs.`)) return;
    try {
  ...
  async function handleResetPin(driver: Driver) {
    if (!confirm(`Reset PIN for ${driver.displayName}? They will need to change it on next login.`)) return;
    try {
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setEditDriver(undefined); setShowForm(true); }}>
          + Add Driver
        </Button>
      </div>
      ...
      {showForm && (
        <div className="card p-6 mb-6">
          <h2 className="font-bold text-primary mb-4">
            {editDriver ? `Edit — ${editDriver.displayName}` : "Add New Driver"}
          </h2>
          <DriverForm
            initial={editDriver}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <div className="text-sm text-muted mb-4">
            {drivers.length === 0 ? "Add your first driver to get started" : "Try a different search or filter"}
          </div>
          {drivers.length === 0 && (
            <Button onClick={() => setShowForm(true)}>Add First Driver</Button>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(driver => (
            <DriverCard
              key={driver.id}
              driver={driver}
              onEdit={() => { setEditDriver(driver); setShowForm(true); }}
              onToggleStatus={() => handleToggleStatus(driver)}
              onResetPin={() => handleResetPin(driver)}
            />
          ))}
        </div>
      )}
