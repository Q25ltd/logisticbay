...
| Term | Meaning |
|---|---|
| `ready` | **Gate** — true only when every *hard* check passes. Not a percentage. (An empty run is one of those hard checks — `stops_assigned` — so `ready` has exactly one rule and an unready run always names a blocker.) |
| `blockers` | Human-readable hard failures (the reasons publish is blocked). Never empty while `ready` is false. |
| `resources.checks` | `{ key, label, status, hard, reason, source }[]` — stops_assigned, driver assigned/available/licence/ADR/trailer-capability, trailer assigned/compatible/available, vehicle assigned/compatible, equipment, mot_inspection, vor_defects, driver_hours. `source` (`allocation` \| `driver` \| `fleet` \| `job` \| `planning`) names where the information is fixed (four intake gates, this screen, or the Planning board). |
| `stops_assigned` (check) | Hard: a run with no active `RunAssignment` has nothing to execute. Source `planning` — stops are put on runs from the Planning board. Added 2026-07-26: `ready` previously went false on an empty run without any check failing, so the planner saw "Not ready" with an empty blocker list and publish answered with a bare "run is not ready". |
| `driver_hours` (check) | REAL since 2026-07-18: estimated run duty (drive + dwell + legal breaks + window waits, `checkRun` offline mode — haversine, no network) vs the driver's day: `ShiftPreference.requestedHours` for that date, else `DriverProfile.minHoursPerDay`. Driving >10h or duty >~13h = hard fail; longer than the driver's day, the 9–10h extension band, or availability-plan "unavailable" = warn; stops without map pins = honest unknown pointing at the job form; **no stops at all = `na`** (the empty run is `stops_assigned`'s to report, not the driver's day). |
